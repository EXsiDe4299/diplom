...
# TODO: сделать нормальный нейминг, поправить баги, убрать повторы кода. В целом, провести рефакторинг
...
@app.get('/mssql/database/get-connection-string')
async def mssql_db_get_conn_str(data: DatabaseInteractionScheme, sqlite_session=sqlite_db_dependency):
    ...
    # следующий код вызывает unclosed connection. нужно пофиксить
...
@app.get('/postgresql/database/get-connection-string')
...
async def mariadb_user_create(user_data: CreateUserScheme, sqlite_session=sqlite_db_dependency,
    ...
    has_account = await sqlite_session.execute(select(Account).join(User).
                                               filter(User.user_telegram_id == user_data.user_telegram_id).
                                               filter(Account.account_type_id == 3))
    has_account = has_account.first()
    ...
    await mariadb_session.execute(text(
        f"""CREATE USER '{user_data.user_login}'@localhost IDENTIFIED BY '{user_password}';"""  # поменять
    ))
    await sqlite_session.commit()
...
async def mariadb_db_create(data: DatabaseInteractionScheme,
                            ...
                            sqlite_session=sqlite_db_dependency):
    try:
        await autocommit_mariadb_session.execute(text(f"CREATE DATABASE {data.database_name}"))
        await autocommit_mariadb_session.execute(
    ...
    new_database = Database(database_name=data.database_name, database_type_id=3)
    sqlite_session.add(new_database)
    await sqlite_session.flush()
    database_id = new_database.database_id
...
@app.get('/mariadb/database/get-connection-string')
