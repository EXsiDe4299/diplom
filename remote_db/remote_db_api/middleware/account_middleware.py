...
async def create_new_account(*, user_id: int, user_data: CreateAccountScheme, sqlite_session: AsyncSession,
    ...
    account_type_id = db_toolkit_data[dbms_name]['account_type_id']
    create_user_query = db_toolkit_data[dbms_name]['create_user_query'](account_login=user_data.account_login,
...
async def change_account(user_id: int, user_data: EditAccountScheme, sqlite_session: AsyncSession,
                         session: AsyncSession):
    dbms_name = session.get_bind().name
    account_type_id: int = db_toolkit_data[dbms_name]['account_type_id']
    if user_data.account_login != user_data.new_account_login:
        await session.execute(text(db_toolkit_data[dbms_name]['edit_login_query'](user_login=user_data.account_login,
                                                                                  new_account_login=user_data.new_account_login)))
    if user_data.account_password != user_data.new_account_password:
        await session.execute(
            text(db_toolkit_data[dbms_name]['edit_password_query'](new_account_login=user_data.new_account_login,
